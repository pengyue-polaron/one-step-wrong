...
- The local Codex SDK adapter is an explicit development-only fallback for reviewed-scenario matching and copy adaptation, dialogue, debrief, and Evidence Coach. It must remain disabled in production, prefer a configured Platform API key, use the authenticated local `codex login`, run in a temporary read-only workspace with isolated configuration, disable shell, browser, plugin, multi-agent, web-search, and tool-network capabilities, and delete the temporary runtime after each request.
- Never route institution research through local Codex or invent source records for it. Research requires the Responses Web Search evidence from the same request. Never expose the local Codex adapter through a public or multi-user server.
...
- Adaptive requests may include a public institution name, official domain, public policy text, sanitized fictional teaching brief, bounded role cards, and the minimum sanitized conversational context needed for the current turn. They must never include private organizational data or personal data, and must use bounded payloads, timeouts, runtime validation, source review, and a deterministic offline fallback.
