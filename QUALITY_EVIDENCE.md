...
| OpenAI-compatible provider configuration | HTTPS-only base URL validation, explicit model selection, OpenRouter server-side Web Search mapping, approved-domain filters, same-response citation checks, and bounded reviewed-scenario adaptation | Verified in adapter tests; live behavior requires provider acceptance check |
