...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "The canonical trace is invalid." }, { status: 400 });
  try {
    return NextResponse.json(await createDebrief(parsed.data));
  } catch {
    return NextResponse.json({ error: "The canonical trace could not be evaluated." }, { status: 400 });
  }
}
