  ...
  return (
    <WindowFrame id="it-report" title="NYU IT Service Desk" icon={<TicketCheck size={15} />} address="https://www.nyu.edu/it/report">
      <div className="support-page">
        ...
        {state.itReported ? (
          <div className="ticket-success" data-testid="ticket-success"><CheckCircle2 size={31} /><h3>工单已创建</h3><p>NYU IT 将协助检查账号与设备。</p><dl><div><dt>工单编号</dt><dd>NYU-IT-2027</dd></div><div><dt>状态</dt><dd>已受理</dd></div></dl></div>
        ) : inIncident ? (
          <form className="report-form" onSubmit={(event) => { event.preventDefault(); dispatch({ type: "RESPONSE_ACTION", action: "report-it" }); }}>
            <div className="form-grid">
              ...
              <label><span>网络名称</span><input value="NYU_Free_5G" readOnly /></label>
            ...
            <fieldset className="report-checks"><legend>事件经过</legend>
              <label><input type="checkbox" defaultChecked /><span><Check size={12} /></span>在网络页面输入过 NYU NetID</label>
            ...
            <label className="report-description"><span>简短描述</span><textarea value="我连接了名为 NYU_Free_5G 的开放网络，并按照页面提示安装了网络配置。之后出现未知设备登录和异常消息。" readOnly /></label>
